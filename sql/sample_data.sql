...
-- 2. Insert Students  
INSERT INTO users (id, username,full_name,  email, password_hash, role, created_at, updated_at) VALUES
('student-001', 'alice_smith','alice_smith', 'alice@bkis.edu', '$2a$10$example_hash_student', 'STUDENT', NOW(), NOW()),
('student-002', 'bob_johnson','bob_johnson', 'bob@bkis.edu', '$2a$10$example_hash_student', 'STUDENT', NOW(), NOW()),
('student-003', 'carol_white','carol_white', 'carol@bkis.edu', '$2a$10$example_hash_student', 'STUDENT', NOW(), NOW());

select * from users where full_name = 'john_doe';
-- 3. Insert Course (with 3 enrolled students)
...
('student-001', @course_id, 149.99, 'COMPLETED', NOW(), NOW()),
('student-002', @course_id, 149.99, 'COMPLETED', NOW(), NOW()),
('student-003', @course_id, 149.99, 'COMPLETED', NOW(), NOW());
...
-- 7. Insert Enrollments (Link students to course)
INSERT INTO enrollments (student_id, course_id, payment_id, status, enrolled_at, expires_at, created_at, updated_at) VALUES
('student-001', @course_id, 2, 'ACTIVE', NOW(), DATE_ADD(NOW(), INTERVAL 1 YEAR), NOW(), NOW()),
('student-002', @course_id, 3, 'ACTIVE', NOW(), DATE_ADD(NOW(), INTERVAL 1 YEAR), NOW(), NOW()),
('student-003', @course_id, 4, 'ACTIVE', NOW(), DATE_ADD(NOW(), INTERVAL 1 YEAR), NOW(), NOW());
...
-- Student 1 progress
('student-001', 1, 450, 1, NOW(), NOW()),      -- Completed first video
('student-001', 2, 200, 0, NOW(), NOW()),      -- Partially watched second video
('student-001', 3, 380, 1, NOW(), NOW()),      -- Completed third video
-- Student 2 progress
('student-002', 1, 100, 0, NOW(), NOW()),      -- Just started
-- Student 3 progress
('student-003', 1, 450, 1, NOW(), NOW()),
('student-003', 2, 520, 1, NOW(), NOW());
...
('student-001', @course_id, 5, 'Excellent course! Very comprehensive and easy to follow.', NOW(), NOW()),
('student-002', @course_id, 4, 'Great content, could use more real-world projects.', NOW(), NOW());
...
-- Course: Java Spring Boot Masterclass - $149.99 (3 students enrolled)
-- Lessons: 3 lessons with 6 videos total
-- Enrollments: 3 active enrollments
-- Payments: 3 completed payments
-- Progress: Various students watching videos
